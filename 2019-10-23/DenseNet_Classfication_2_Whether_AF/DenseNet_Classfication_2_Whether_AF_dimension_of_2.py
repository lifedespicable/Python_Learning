...
train_data_n_Arr = np.append(train_data_normal[:75*2], train_data_Arr[:30*2])
train_data_u_Wr = np.append(train_data_Su[:1*2], train_data_Wr[:1*2])
train_data_Sb_Ab = np.append(train_data_Sb[:1*2], train_data_Ab[:4*2])
Train_Data_Other = train_data_other[:174*2]
train_data_n_arr_su_wr = np.append(train_data_n_Arr, train_data_u_Wr)
train_data_sb_ab_other = np.append(train_data_Sb_Ab, Train_Data_Other)
train_data_Not_AF = np.append(train_data_n_arr_su_wr, train_data_sb_ab_other)
train_data_Whether_AF = np.append(train_data_AF, train_data_Not_AF)
Train_Data_Whether_AF = train_data_Whether_AF.reshape(-1, 1, 6000)

label_data_AF = np.ones(572)
label_data_Not_AF = np.zeros(572)
Label_data_train = np.append(label_data_AF, label_data_Not_AF)
...
X_train,X_valid,y_train,y_valid = train_test_split(Train_Data_Whether_AF,Label_for_train,test_size=0.3,random_state=0)
# print(X_train.shape)

checkpoint = keras.callbacks.ModelCheckpoint('DenseNet_Classfication_2_Whether_AF.h5', monitor='val_acc', verbose=1, save_best_only=True)
