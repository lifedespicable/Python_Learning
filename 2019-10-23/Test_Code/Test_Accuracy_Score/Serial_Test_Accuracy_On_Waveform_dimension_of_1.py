...
def Serial_Test_Accuracy_On_Waveform(X_test, cols):
    ...
    Result = []
    label_V_W = model_V_W.predict(X)
    label_S = model_S.predict(X)
    label_BBB = model_BBB.predict(X)
    label_N = model_N.predict(X)
    for i in range(label_BBB.shape[0]):
        if(label_V_W[i][1] > label_V_W[i][0]):
            Result.append(1)
        elif(label_S[i][1] > label_S[i][0]):
            Result.append(0)
        elif(label_BBB[i][1] > label_BBB[i][0]):
            Result.append(3)
        else:
            Result.append(2)
...
Result_All = Serial_Test_Accuracy_On_Waveform(Test_Data_All, 800)
