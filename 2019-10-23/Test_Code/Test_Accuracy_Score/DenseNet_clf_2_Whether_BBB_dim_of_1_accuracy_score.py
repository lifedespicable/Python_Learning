...
test_data_N = np.append(test_N_except_VN, train_data_VN[77*2 : 96*2]).reshape(-1, 800)
test_data_S = train_data_S[543*2:679*2].reshape(-1, 800)[:46*2]
test_data_BBB = train_data_BBB[543 * 2:].reshape(-1, 800)[:136*2]
test_data_V_W = train_data_V[540*2:675*2].copy().reshape(-1, 800)
test_data_N_VW = np.append(test_data_N[:45*2], test_data_V_W[:45*2])
test_data_except_BBB = np.append(test_data_N_VW, test_data_S)
test_data_ALL = np.append(test_data_BBB, test_data_except_BBB)
...
model_S = load_model('DenseNet_Classfication_2_Whether_S_qrs_dimension_of_1_0.98014.h5')
...
def Serial_Test_Accuracy_On_Waveform(X_test, cols):
    ...
    for i in range(label_BBB.shape[0]):
        if(label_BBB[i][1] > label_BBB[i][0]):
            Result.append(1)
        else:
            Result.append(0)
