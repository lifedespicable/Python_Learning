...
train_data_N_BBB = np.append(train_data_BBB[:49*2], train_data_normal[:520*2])
train_data_P_St = np.append(train_data_P[:2*2], train_data_St[:96*2])
train_data_Stb_T = np.append(train_data_Stb[:4*2], train_data_T[:93*2])
train_data_V_W = np.append(train_data_V, train_data_W)
train_data_S_Other = np.append(train_data_S[:394*2], train_data_Other[:256*2])
train_data_BBB_N_P_St = np.append(train_data_N_BBB, train_data_P_St)
train_data_Stb_T_S_Other = np.append(train_data_Stb_T, train_data_S_Other)
train_data_except_V_W = np.append(train_data_BBB_N_P_St, train_data_Stb_T_S_Other)
train_data_Whether_V_W_qrs = np.append(train_data_V_W, train_data_except_V_W)
Train_Data_Whether_V_W_qrs = train_data_Whether_V_W_qrs.reshape(-1, 1, 800)

label_data_S = np.ones(1414*2)
label_data_Not_S = np.zeros(1414*2)
...
X_train,X_valid,y_train,y_valid = train_test_split(Train_Data_Whether_V_W_qrs,Label_for_train,test_size=0.3,random_state=0)
# print(X_train.shape)

checkpoint = keras.callbacks.ModelCheckpoint('DenseNet_Classfication_2_Whether_V_W_qrs.h5', monitor='val_acc', verbose=1, save_best_only=True)
