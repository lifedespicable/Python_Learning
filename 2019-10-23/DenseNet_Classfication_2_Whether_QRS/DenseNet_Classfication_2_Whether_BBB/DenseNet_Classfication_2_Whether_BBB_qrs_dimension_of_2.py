...
train_data_N_S = np.append(train_data_S[:114*2], train_data_normal[:151*2])
train_data_P_St = np.append(train_data_P[:1*2], train_data_St[:28*2])
train_data_Stb_T = np.append(train_data_Stb[:1*2], train_data_T[:27*2])
train_data_V_W = np.append(train_data_V[:45*2], train_data_W[:1*2])
train_data_S_N_P_St = np.append(train_data_N_S, train_data_P_St)
train_data_Stb_T_V_W = np.append(train_data_Stb_T, train_data_V_W)
train_data_except_other = np.append(train_data_S_N_P_St, train_data_Stb_T_V_W)
train_data_Not_BBB_qrs = np.append(train_data_except_other, train_data_Other[:74*2])
train_data_Whether_BBB_qrs = np.append(train_data_BBB, train_data_Not_BBB_qrs)
Train_Data_Whether_BBB_qrs = train_data_Whether_BBB_qrs.reshape(-1, 1, 800)

label_data_S = np.ones(442*2)
label_data_Not_S = np.zeros(442*2)
...
X_train,X_valid,y_train,y_valid = train_test_split(Train_Data_Whether_BBB_qrs,Label_for_train,test_size=0.3,random_state=0)
# print(X_train.shape)

checkpoint = keras.callbacks.ModelCheckpoint('DenseNet_Classfication_2_Whether_BBB_qrs.h5', monitor='val_acc', verbose=1, save_best_only=True)
