...
train_data_Other_BBB = np.append(train_data_BBB[:219*2], train_data_Other[:1145*2])
train_data_P_St = np.append(train_data_P[:6*2], train_data_St[:429*2])
train_data_Stb_T = np.append(train_data_Stb[:17*2], train_data_T[:418*2])
train_data_V_W = np.append(train_data_V[:697*2], train_data_W[:3*2])
train_data_BBB_Other_P_St = np.append(train_data_Other_BBB, train_data_P_St)
train_data_Stb_T_V_W = np.append(train_data_Stb_T, train_data_V_W)
train_data_except_N_S = np.append(train_data_BBB_Other_P_St, train_data_Stb_T_V_W)
train_data_except_N = np.append(train_data_except_N_S, train_data_S[:1762*2])
train_data_Whether_N_qrs = np.append(train_data_normal, train_data_except_N)
Train_Data_Whether_N_qrs = train_data_Whether_N_qrs.reshape(-1, 1, 800)

label_data_S = np.ones(4696*2)
label_data_Not_S = np.zeros(4696*2)
...
X_train,X_valid,y_train,y_valid = train_test_split(Train_Data_Whether_N_qrs,Label_for_train,test_size=0.3,random_state=0)
# print(X_train.shape)

checkpoint = keras.callbacks.ModelCheckpoint('DenseNet_Classfication_2_Whether_N_qrs.h5', monitor='val_acc', verbose=1, save_best_only=True)
