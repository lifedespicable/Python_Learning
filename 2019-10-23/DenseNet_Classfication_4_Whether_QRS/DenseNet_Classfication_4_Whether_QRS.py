...
label_trian_data_N = np.zeros(543*2, dtype = int)
label_train_data_VW = np.ones(543*2 ,dtype = int)
label_train_data_S = np.full(fill_value = 2, shape = 543*2, dtype = int)
label_train_data_BBB = np.full(fill_value = 3, shape = 543*2, dtype = int)
Label_data_train = np.concatenate([label_trian_data_N, label_train_data_VW, label_train_data_S, label_train_data_BBB])
...
Label_data_test = np.concatenate([label_test_data_N, label_test_data_VW, label_test_data_S, label_train_data_BBB])
...
checkpoint = keras.callbacks.ModelCheckpoint('DenseNet_Classfication_4_Whether_qrs_dimension_of_1_depth_10_Subject_Oriented.h5', monitor='val_acc', verbose=1, save_best_only=True)
